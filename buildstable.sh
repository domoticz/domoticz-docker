...
echo "docker buildx build --push --no-cache --platform $BUILDX_PLATFORMS --build-arg RELEASE=$RELEASE --build-arg APP_VERSION=$APPVERSION --build-arg APP_HASH=$APPHASH --build-arg BUILD_DATE=$RELEASE_DATE --tag domoticz/domoticz:stable --tag domoticz/domoticz:$VERSION ."
